	...
	<div class="p-3 mb-3 border">
		...
		<?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) : ?>
			<div class="d-flex flex-row justify-content-between border-bottom py-2">
				<div><h6 class="m-0"><?php wc_cart_totals_coupon_label( $coupon ); ?></h6></div>
		...
		<?php foreach ( WC()->cart->get_fees() as $fee ) : ?>
			<div class="d-flex flex-row justify-content-between border-bottom py-2">
				<div><h6 class="m-0"><?php echo esc_html( $fee->name ); ?></h6></div>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
				foreach ( WC()->cart->get_tax_totals() as $code => $tax ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited
					?>
					<div class="d-flex flex-row justify-content-between border-bottom py-2">
						<div><h6 class="m-0"><?php echo esc_html( $tax->label ) . $estimated_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h6></div>
				...
				?>
				<div class="d-flex flex-row justify-content-between border-bottom py-2">
					<div><h6 class="m-0"><?php echo esc_html( WC()->countries->tax_or_vat() ) . $estimated_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h6></div>
		...
		<div class="d-flex flex-row justify-content-between pt-2">
			<div><h6 class="m-0"><?php esc_html_e( 'Total', 'woocommerce' ); ?></h6></div>
